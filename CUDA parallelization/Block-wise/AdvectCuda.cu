...
void Advect3D::updateLW2D_tmr_Cuda(HaloArray3D* u){
    ...
    LW2DtmrKernel1<<<gridSize, blockSize>>>(u, size);
    ...
    LW2DtmrKernel2<<<gridSize, blockSize>>>(u, d_uh1, Ux, Vy, 0, size);
    cudaDeviceSynchronize();
    LW2DtmrKernel2<<<gridSize, blockSize>>>(u, d_uh2, Ux, Vy, 1, size);
    cudaDeviceSynchronize();
    LW2DtmrKernel2<<<gridSize, blockSize>>>(u, d_uh3, Ux, Vy, 2, size);
    cudaDeviceSynchronize();
    LW2DtmrKernel3<<<gridSize, blockSize>>>(u, d_uh1, d_uh2, d_uh3, size);
    cudaDeviceSynchronize();
...
__global__ void LWN2kernel1(HaloArray3D* u, HaloArray3D* uh, double Ux, double Uy, int optimized, double cim1,
        double ci0, double cip1, double cjm1, double cj0, double cjp1, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;

    for(int p = j0; p < u->l.y / size + 1; p += dj){
        for(int q = i0; q < u->l.x / size + 1; q += di){
...
__global__ void LWN2kernel2(HaloArray3D* u, HaloArray3D* uh, int optimized, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;
    for(int p = j0; p < u->l.y / size + 1; p += dj){
        for(int q = i0; q < u->l.x / size + 1; q += di){
...
void Advect3D::updateLWN2Cuda(HaloArray3D *u) {
    ...
    LWN2kernel1<<<gridSize, blockSize>>>(u, d_uh, Ux, Uy, 1, cim1, ci0, cip1, cjm1, cj0, cjp1, size);
    cudaDeviceSynchronize();
    LWN2kernel2<<<gridSize, blockSize>>>(u, d_uh, 1, size);
    cudaDeviceSynchronize();
    cudaFree(d_uh);
    cudaFree(uuh);
} //updateLWN2()
...
__global__ void LWN4kernel1(HaloArray3D* u, HaloArray3D* uh, double Ux, double Uy, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;
    for(int p = j0; p < u->l.y / size + 1; p += dj){
        for(int q = i0; q < u->l.x / size + 1; q += di){
...
__global__ void LWN4kernel2(HaloArray3D* u, HaloArray3D* uh, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;
    for(int p = j0; p < u->l.y / size + 1; p += dj){
        for(int q = i0; q < u->l.x / size + 1; q += di){
...
void Advect3D::updateLWN4Cuda(HaloArray3D *u){
    ...
    LWN4kernel1<<<gridSize, blockSize>>>(u, d_uh, Ux, Uy, size);
    cudaDeviceSynchronize();
    LWN4kernel2<<<gridSize, blockSize>>>(u, d_uh, size);
    cudaDeviceSynchronize();
    cudaFree(d_uh);
    cudaFree(uuh);
}

__global__ void NN2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
...
__global__ void NW2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
...
__global__ void NF2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
...
__global__ void WN2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;
    for(int p = j0; p < u->l.y / size + 1; p += dj){
        for(int q = i0; q < u->l.x / size + 1; q += di){
...
__global__ void WW2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
...
__global__ void WF2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;
...
__global__ void FN2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
    int i0 = blockIdx.x * blockDim.x + threadIdx.x, di = blockDim.x*gridDim.x;
    int j0 = blockIdx.y * blockDim.y + threadIdx.y, dj = blockDim.y*gridDim.y;
    for(int p = j0; p < u->l.y / size + 1; p += dj){
        for(int q = i0; q < u->l.x / size + 1; q += di){
...
__global__ void FW2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
...
__global__ void FF2Kernel(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, int size){
...
__global__ void LW2Dkernel1(HaloArray3D* u, HaloArray3D* uh, double Ux, double Vy, unsigned int opt_stset, int size){
...
__global__ void LW2Dkernel2(HaloArray3D* u, HaloArray3D* uh, int size){
...
void Advect3D::updateLW2DCuda(HaloArray3D *u){
    ...
    switch (opt_stset){
        case STB_NN2:
            NN2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_NW2:
            NW2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_NF2:
            NF2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_WN2:
            WN2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_WW2:
            WW2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_WF2:
            WF2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_FN2:
            FN2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_FW2:
            FW2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        case STB_FF2:
            FF2Kernel<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, size);
            break;
        default:
            LW2Dkernel1<<<gridSize, blockSize>>>(u, d_uh, Ux, Vy, opt_stset, size);
    }

    cudaDeviceSynchronize();
    LW2Dkernel2<<<gridSize, blockSize>>>(u, d_uh, size);
    cudaDeviceSynchronize();
    cudaFree(d_uh);
    cudaFree(uuh);
}
